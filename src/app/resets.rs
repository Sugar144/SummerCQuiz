...
impl QuizApp {
    ...
    pub fn reiniciar_semana(&mut self, module_idx: usize) {
        ...
        let first_pending = self.quiz.modules.get(module_idx).and_then(|module| {
            module.levels.iter().enumerate().find_map(|(li, lvl)| {
        ...
        if let Some(module) = self.quiz.modules.get_mut(module_idx) {
    ...
    pub fn reiniciar_nivel(&mut self, module_idx: usize, level_idx: usize) {
        let lang = self.selected_language.unwrap_or(Language::C);
