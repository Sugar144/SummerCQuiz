...
use crate::view_models::ModuleInfo;
use egui::{Align, CentralPanel, Context};

pub fn ui_module_menu(app: &mut QuizApp, ctx: &Context) {
    CentralPanel::default().show(ctx, |ui| {
        let max_width = 400.0;
        let content_width = ui.available_width().min(max_width);
        let button_h = 40.0;

        let infos: Vec<ModuleInfo> = app.module_infos();

        // centrar…
        let modules_count = infos.len() as f32;
        let estimated_h = 80.0 + (button_h + 8.0) * (modules_count + 1.0);
                    ...
                    ui.with_layout(egui::Layout::top_down(Align::Center), |ui| {
                        ...
                        for info in &infos {
                            ...
                            if clicked_main && info.unlocked {
                                app.progress_mut().current_module = Some(info.idx);
                                app.open_level_menu();
                            }
